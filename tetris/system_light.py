...
class LightSystem:
    ...
    _garbage_new_line = ["G" for _ in range(10)]
    _field_new_line = [False for _ in range(10)]
        
    def __init__(self, w, h, preview_num=5, init=True):
        """
    ...
    def init(self):
        ...
        self._init_mino_pos = Pos(self.w//2-1, self.h_padding)
        self._bag, self._next_bag = self._gen_bag(),self._gen_bag()
    ...
    def clone(self):
        newsys = LightSystem(self.w, self.h, self.preview_num, False)

        newsys.curr_mino = self.curr_mino.clone()

        ## field
        newsys.field = [line[:] for line in self.field]
        
        newsys._init_mino_pos = Pos(self.w//2-1, self.h_padding)
        newsys._bag, newsys._next_bag = newsys._gen_bag(),newsys._gen_bag()

        ## fps
        newsys.spend_second = self.spend_second

        ## hold system
        newsys._curr_mino_num = self._curr_mino_num
        newsys._hold_mino_num = self._hold_mino_num
        newsys._hold_used = self._hold_used

        ## score system
        newsys.used_mino_count = self.used_mino_count
        newsys.combo_count = self.combo_count
        newsys.cleaned_line = self.cleaned_line

        newsys._b2b = self._b2b
        newsys.total_score = self.total_score
        newsys._last_score = self._last_score
        newsys._judgement = self._judgement
        newsys._last_rot_point = self._last_rot_point  # remembers last rotation SRS state

        newsys.last_line_cleared = self.last_line_cleared
        newsys.last_lines_sent = self.last_lines_sent
        newsys.total_lines_sent = self.total_lines_sent
        newsys.total_lines_recv = self.total_lines_recv
        newsys._garbage_delay = self._garbage_delay
        newsys.garbage_pos = self.garbage_pos
        newsys.garbage_pos_prob = 0.1
        newsys.last_line_down = self.last_line_down

        newsys.receive_queue = deque(self.receive_queue)
        newsys.receive_queue_lines = self.receive_queue_lines
        newsys.incoming_garbage_next = self.incoming_garbage_next
        newsys.outgoing_garbage = self.outgoing_garbage
        newsys.max_mino_ops = self.max_mino_ops
        newsys._cur_mino_ops = self._cur_mino_ops
        return newsys
    ...
    def get_current_mino(self):
        send_mino = self.curr_mino.clone()
    ...
    def get_ghost_mino(self):
        ghost_mino = self.curr_mino.clone()
        count = 0
    ...
    def _is_enable_rotate(self, rotated_direction=CW):
        blocks = self._copyblock(self.curr_mino.blocks)
        for block in blocks:
        ...
        for point, rotate_offset_option in enumerate(
            ...
            _offset_blocks = self._copyblock(blocks)
    ...
    def _apply_garbage_lines(self, line_sent):
        ...
        if diff > 0:
            ...
            for i in range(diff):
                ...
                del self.field[0]
                newline = [self._garbage_new_line[:]]
    ...
    def _add_new_line(self):
        new_field = [self._field_new_line[:]]
    ...
    def _is_enable_move_y(self):
        for block in self._copyblock(self.curr_mino.blocks):
            block.y += 1
    ...
    @staticmethod
    def _gen_bag(): #5040
        bag_id = randint(0, 5039)
        div = [720, 120, 24, 6, 2, 1] 
        bag = []
        pos = [0, 1, 2, 3, 4, 5, 6]
        for i in range(6):
            mi = bag_id//div[i]
    ...
    @staticmethod
    def _copyblock(blocks):
        return [Pos(b.x, b.y) for b in blocks]
