...
class Painter:
    ...
    def init(self):
        self._grid_list = [
            pg.Rect(x * self.u, y * self.u, self.u, self.u)
            for x in range(self.w)
            for y in range(self.h)
        ]
        self._death_zone_list = [
            pg.Rect((self._get_w_center() - 2 + i) * self.u, 0, self.u, self.u)
            for i in range(4)
        ]
    ...
    def draw_grid(self, screen):
        [
            self._draw_rect(screen, self._grid_color, grid, self._grid_width)
            for grid in self._grid_list
        ]
        [
            self._draw_rect(
                screen, self._death_zone_color, death_zone, self._grid_width * 2
            )
            for death_zone in self._death_zone_list
        ]
    ...
    def draw_ghost_mino(self, screen, ghost_mino: Mino):
        self._draw_mino(
            screen,
            ghost_mino,
            color=self._get_half_tone_color(self._get_mino_color(ghost_mino)),
        )
    ...
    def draw_hold_mino(self, screen, hold_mino: Mino):
        ...
        self._draw_mino(
            screen,
            hold_mino,
            offset=Pos(2 - hold_mino.center.x, hold_mino.y_offset + 1.5),
        )
        ...
        for y, row in enumerate(field):
            for x, cell in enumerate(row):
                if cell is not False:
                    self._draw_rect(
                        screen, self._color_map[cell], self._get_rect_printer(x, y)
                    )

    def draw_preview_mino_list(self, screen, mino_list: List[Mino]):
        for i, mino in enumerate(mino_list):
            self._draw_mino(
                screen, mino, offset=Pos(2 - mino.center.x, i * 3 + mino.y_offset + 1.5)
            )

    def _draw_mino(
        self, screen, mino: Mino, color: tuple | None = None, offset: Pos = Pos(0, 0)
    ):
        ...
        for block in mino.blocks:
            self._draw_rect(
                screen,
                color,
                self._get_rect_printer(block.x + offset.x, block.y + offset.y),
            )
